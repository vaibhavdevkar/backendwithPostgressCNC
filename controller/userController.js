...
exports.updateUser = async (req, res) => {
  ...
  let picture, documents;
  if (req.files.picture) {
    await deleteOldFiles(id);
    picture = req.files.picture[0].path;
  } else {
    const { rows } = await pool.query(
      'SELECT picture FROM user_master WHERE user_id = $1;',
      [id]
    );
  ...
  if (req.files.documents) {
    ...
    const { rows } = await pool.query(
      'SELECT documents FROM user_master WHERE user_id = $1;',
      [id]
    );
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      'DELETE FROM user_master WHERE user_id = $1;',
      [id]
    );
